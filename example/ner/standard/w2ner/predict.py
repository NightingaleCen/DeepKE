...
def main(cfg):
    ...
    total_text_num = sum([len(predict_data[plant])
    ...
    for plant, texts in predict_data.items():
        ...
        for entity_text in texts:
            ...
            bert_inputs = np.array([tokenizer.cls_token_id] +
                                   bert_inputs + [tokenizer.sep_token_id])
            ...
            with torch.no_grad():
                bert_inputs = torch.tensor(
                    [bert_inputs], dtype=torch.long).cuda()
                grid_mask2d = torch.tensor(
                    [grid_mask2d], dtype=torch.bool).cuda()
                dist_inputs = torch.tensor(
                    [dist_inputs], dtype=torch.long).cuda()
                pieces2word = torch.tensor(
                    [pieces2word], dtype=torch.bool).cuda()
                sent_length = torch.tensor(
                    [sent_length], dtype=torch.long).cuda()
                ...
                for ner in decode_entities:
                    ...
                    entity = ''.join([input_sentence[ner_index]
                                      for ner_index in ner_indexes])
